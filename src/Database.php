...
class Database
{
    ...
    public function __construct($root, $kinds)
    {
        ...
        while ($input) {
            ...
            $more = $this->Read('Tambahkan koneksi lain? (y/n)');
            if ($more !== 'y') {
                $input = false;
            }
        }
    ...
    public function Setup($root, $db, $host, $port, $dbName, $user, $pass, $schema)
    {
        switch ($db) {
            ...
            case 'mongo':
                ...
                break;
            default:
                die($this->Prints(sprintf("Sorry, database '%s' not yet supported.", $db)));
        }
    ...
    public function SetupMySQL($host, $port, $dbName, $user, $pass)
    {
        try {
            ...
            die($this->Prints("Failed to connect. Check database setting or make sure driver pdo_odbc installed!"));
        }
    ...
    {
        return $this->Prints('Database setting completed');
    }
